...
import { useStaffProfilesFiltered } from "@/hooks/useApi";
import { apiClient } from "@/lib/api-client";
import { StaffProfile } from "@/lib/types";

type StatusFilter = "ALL" | "ACTIVE" | "INACTIVE";

type StaffRole = "STAFF" | "ADMINISTRATIVE";
...
export default function StaffProfilesPage() {
  ...
  const paginatedRows = useMemo(() => filtered.slice((page - 1) * size, page * size), [filtered, page, size]);
  ...
  const handleOpenEdit = (s: StaffProfile) => {
    setEdit(s);
    const currentRole = ((s as any).role || (s as any).userRole || "").toString().trim().toUpperCase();
    if (currentRole === "ADMINISTRATIVE") setRole("ADMINISTRATIVE");
    else setRole("STAFF");
    setEditOpen(true);
  };
  ...
  const save = async () => {
    ...
    // Email is required for creating (to resolve accountId)
    const workEmail = (edit.workEmail || "").toString().trim();
    if (!edit.id) {
      if (!workEmail) {
        setFlash({ type: "error", text: "Vui lòng nhập Email để tạo nhân viên." });
        return;
      }
      ...
      // Edit: if email is provided, validate format
    ...
    try {
      ...
      if (!accountId) {
        ...
        if (workEmail) {
          const resolved = await resolveAccountIdByEmail(workEmail);
          if (!resolved) {
      ...
      const response = edit.id
        ? await apiClient.updateStaffProfile(edit.id, payload as any)
        : await apiClient.createStaffProfile(payload as any);
  ...
  return (
    <div className="px-6 pt-4 pb-6">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="border-b border-gray-200/50 px-6 py-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <Button className="h-10 px-4 bg-blue-600 text-white hover:bg-blue-700 rounded-xl text-sm whitespace-nowrap" onClick={handleOpenCreate}>
                  Thêm nhân viên
                </Button>
              </div>
            </div>
                  ...
                  >
                    ...
                    <div className="px-3 py-3 bg-gray-50 border-t border-gray-100">
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <Button variant="danger" className="h-10 text-xs font-medium px-2" onClick={() => s.id !== undefined && doDelete(s.id)}>
                          Xóa
                        </Button>
                      </div>
                    </div>
              ...
              {filtered.length > 0 && (
                <div className="bg-gradient-to-r from-gray-50 to-blue-50 px-3 py-4 border-t border-gray-200/50">
                  <div className="flex flex-col lg:flex-row items-center justify-between gap-3 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="secondary" className="h-9 px-3 text-sm" disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>
                        Trước
                      </Button>
            ...
            <div className="space-y-4 p-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div className="rounded-lg border border-gray-200 p-3 bg-white">
                  ...
                  <div className="font-medium text-gray-900 break-words">{selected.employeeId}</div>
                </div>
                <div className="rounded-lg border border-gray-200 p-3 bg-white">
        ...
        {/* Edit Modal */}
        <Modal open={editOpen} onClose={handleCloseEdit} title={edit.id ? "Sửa nhân viên" : "Thêm nhân viên"} size="xl">
          <div className="p-4 sm:p-6 space-y-4">
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-3 sm:p-4 border border-blue-200 mb-2">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-base sm:text-lg font-semibold text-gray-900 truncate">{edit.jobTitle || "Nhân sự"}</h3>
                    {renderStatusBadge(edit.isActive)}
                  </div>
                  <div className="text-xs sm:text-sm text-gray-600 truncate">
                    {edit.department || "Chưa có phòng ban"}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1">
                ...
                <select className="w-full h-10 rounded-lg border border-gray-300 bg-white px-3 text-sm" value={role} onChange={(e) => setRole(e.target.value as any)}>
